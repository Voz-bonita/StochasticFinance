...
Após o ajuste do modelo, iniciou-se uma busca por padrões de autocorrelação da série não capturados pelo modelo, sempre sobre as séries diferenciadas. A figura \ref{fig:acf} aponta para autocorrelações contidas dentro das bandas de confiança, indicando ausência de padrões não detectados nesse sentido.

```{r fig.cap="\\label{fig:acf}Correlograma dos retornos diários para o preço de fechamento de PETR4 em 2025 e do modelo ajustado."}
par(mfrow = c(2, 1))
acf(
...
```{r fig.cap="\\label{fig:residual}Gráficos da análise de resíduos em 2025 para o modelo ajustado."}
